...
public class AdminPanelService
{
    ...
    /// <summary>
    /// Deletes a given episodes transcript.
    /// </summary>
    /// <param name="episodeId"></param>
    /// <returns></returns>
    /// <exception cref="Exception"></exception>
    public async Task<bool> DeleteTranscriptAsync(Guid episodeId)
    {
        Episode episode = await _db.Episodes
            .Where(p => p.Id == episodeId)
            .FirstOrDefaultAsync() ?? throw new Exception("Episode does not exist.");

        FileStorageHelper.RemoveTranscript(episodeId, episode.PodcastId);

        if (FileStorageHelper.GetTranscriptStatus(episodeId, episode.PodcastId) == FileStorageHelper.TranscriptStatus.None)
        {
            return true;
        }
