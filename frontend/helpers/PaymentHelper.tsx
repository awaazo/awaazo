...
export default class PaymentHelper {
     ...
     * @returns A BaseResponse object with the server's response.
     */
    public static WithdrawRequest = async (requestData : number): Promise<WithdrawResponse> => {
        // Create the request options.
        const options = {
            method: "POST",
     ...
     * @returns A BaseResponse object with the server's response.
     */
    public static GetAllTransactions = async (page:number,pageSize:number): Promise<TransactionResponse> => {
        // Create the request options.
        const options = {
            method: "GET",
        ...
        try {
            ...
            await Promise.all(requestResponse.data.map(async (element: Transaction) => {
                if (element.type === "Gift") {
                    try {
                        const res = await UserProfileHelper.profileGetByIdRequest(element.senderId);
                        if(res.status == 200){
