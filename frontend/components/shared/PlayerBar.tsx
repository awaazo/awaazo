...
import { Box, Flex, IconButton, Image, Text, Slider, SliderTrack, SliderFilledTrack, SliderThumb, Tooltip, useColorModeValue, useBreakpointValue } from "@chakra-ui/react";
import { FaPlay, FaPause, FaHeart, FaCommentAlt, FaVolumeUp, FaVolumeMute, FaStepForward, FaStepBackward } from "react-icons/fa";
...
const PlayerBar: React.FC<Episode> = ({ coverArt, episodeName = "Unknown Episode", podcaster = "Unknown Podcaster", duration, likes, comments, sections = [] }) => {
  ...
  const skipForward = () => setPosition((prevPos) => Math.min(prevPos + skipAmount, duration));
  const skipBackward = () => setPosition((prevPos) => Math.max(prevPos - skipAmount, 0));
  ...
  return (
    <Box boxSizing="border-box" position="sticky" bottom={4} left={0} right={0} p={4} m={3} borderRadius="2em" bg={useColorModeValue("rgba(255, 255, 255, 0.2)", "rgba(0, 0, 0, 0.2)")} shadow="md" style={{ backdropFilter: "blur(50px)" }} border="3px solid rgba(255, 255, 255, 0.05)" boxShadow="0px 0px 15px rgba(0, 0, 0, 0.4)">
        ...
        {!isMobile && (
          <Flex width="50%" mx={4} alignItems="center">
            ...
            <Slider aria-label="Track Timeline" value={position} max={duration} onChange={(val) => setPosition(val)}>
              ...
              <Tooltip label={getCurrentSectionName()} placement="top" openDelay={900} bg="transparent" color="white" px={2} py={1} borderRadius="xl" fontSize="xs" boxShadow="0px 4px 10px rgba(0, 0, 0, 0.1)">
