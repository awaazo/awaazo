...
const EpisodeMenu: React.FC<EpisodeMenuProps> = ({ episode, inPlaylist, playlistId, onHistoryUpdate }) => {
  ...
  const handleDeleteWatchHistory = async () => {
    try {
      const response = await PodcastHelper.deleteWatchHistory(episode.id);
      if (response.status === 200) {
        toast({
        ...
        onHistoryUpdate(); // Call this function to refresh the history
      } else {
        toast({
          title: "Failed to delete watch history",
          status: "error",
          position: "bottom-right",
          duration: 3000,
          isClosable: true,
      ...
      <Menu isOpen={isMenuOpen}>
        ...
        >
          ...
          <MenuItem
            ...
            backgroundColor="transparent"
          >
            Delete From History <MdDelete size="20px" style={{ marginLeft: "auto", color: "white" }} />
          </MenuItem>
        </MenuList>
      </Menu>
