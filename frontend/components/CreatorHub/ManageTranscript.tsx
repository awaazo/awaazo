    ...
    <Box>
      ...
      >
        {transcriptLines && transcriptLines[0].text ? (
              ...
              {line.text
                ...
                .map((word, wordIndex) =>
                  editState.textIndex === lineIndex && editState.wordIndex === wordIndex ? (
                    <Input value={editState.value} onChange={(e) => handleWordChange(e, lineIndex, wordIndex)} onBlur={() => saveWord(lineIndex, wordIndex)} autoFocus key={wordIndex} size="sm" />
                  ) : (
                    <span onClick={() => setEditState({ textIndex: lineIndex, wordIndex, value: word })} style={{ marginRight: '5px', cursor: 'pointer', display: 'inline-block' }} key={wordIndex}>
                      {word}
                    </span>
                  )
                )}
          ...
          <span>{t('transcript.not_available')}</span>
        )}
      </Box>
      <Box
        style={{
