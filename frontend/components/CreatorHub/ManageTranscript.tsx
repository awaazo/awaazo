...
const saveWord = (textIndex, wordIndex) => {
  if (transcript){
  const updatedTranscript = transcript.map((text, idx) => {
    if (idx === textIndex) {
      const updatedWords = text.words.map((word, index) => {
        if (index === wordIndex) {
          return { ...word, word: editState.value };
        }
        return word;
      });

      // Update the text based on the modified words
      const updatedText = updatedWords.map((word) => word.word).join(' ');
      
      return { ...text, words: updatedWords, text: updatedText };
    }
    return text;
  });

  setTranscript(updatedTranscript);
  setEditState({ textIndex: null, wordIndex: null, value: '' }); // Reset edit state
}
};

 // Function to handle the bookmark/delete bookmark action
 const saveTranscript = () => {
  if (transcript && transcript[0].text) {

  const request: editTranscriptLinesRequest = transcript.map((line) => ({
    start: line.start,
    end: line.end,
    text: line.text,
  ...
  console.log("transcript 3 text: " + transcript[0].text);
...
return (
  ...
  >
    {transcript && transcript[0].text ? (
      transcript.map((text, textIndex) => (
      <Box key={textIndex} style={{ marginBottom: '10px' }}>
        {text.words.map((word, wordIndex) => (
          editState.textIndex === textIndex && editState.wordIndex === wordIndex ? (
            <Input
              value={editState.value}
              onChange={handleWordChange}
              onBlur={() => saveWord(textIndex, wordIndex)}
              autoFocus
              size="sm"
            />
          ) : (
            <span
              onClick={() => setEditState({ textIndex, wordIndex, value: word.word })}
  ...
  >
  <Button variant="gradient" onClick={saveTranscript}>Save Transcript</Button>
  </Box>
  </Box>
);
