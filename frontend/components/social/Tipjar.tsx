...
import { Button, Icon, Tooltip, Modal, ModalOverlay, ModalContent, ModalHeader, ModalCloseButton, ModalBody, ModalFooter, FormControl, RadioGroup, Radio, Stack, Input, VStack, Text, Center } from "@chakra-ui/react";
...
const Tipjar = ({ episodeId }) => {
    ...
    const handleCustomPointsChange = (event) => setCustomPoints(event.target.value);
    ...
    const handleTipSubmit = async () => {
        ...
        try {
            // const response = await PaymentHelper.processTip(episodeId, amount);
    ...
    const supporterMessage = points === "custom" ? calculateSupporterMessage(customPoints) : calculateSupporterMessage(points);
            ...
            {isLoggedIn && (
                <Modal isOpen={isModalOpen} onClose={() => setModalOpen(false)} size="sm" >
                    ...
                    <ModalContent>
                        ...
                        <Text fontSize="sm" color="gray.500" textAlign="center">Select the number of points you would like to gift</Text>
                        <ModalCloseButton />
                        <ModalBody>
                            ...
                            <VStack spacing={5} align="center" p={5}>
                                <RadioGroup onChange={handlePointsChange} value={points}>
                                    <Stack direction="column" align="center">
                                        ...
                                        <Radio value="custom" alignSelf="center">Custom</Radio>
                                    </Stack>
                                </RadioGroup>
                                {points === "custom" && (
                                    <FormControl>
                                ...
                                <Center w="full" justifyContent="center" alignItems="center"><Text textAlign="center">{supporterMessage}</Text></Center>
                            </VStack>
                        </ModalBody>
                        <ModalFooter>
                            <Button leftIcon={<Icon as={PiCurrencyDollarSimpleFill} />} onClick={handleTipSubmit} variant="gradient">
                                Tip Creator
                            </Button>
                        </ModalFooter>
                    </ModalContent>
