...
const Notifications: FC<NotificationsProps> = ({ isOpen, onClose }) => {
  ...
  const episodeInteractions = notifications.filter(
    (notification): notification is UserEpisodeInteraction =>
      "episodeId" in notification &&
      notificationSettings.followedPodcasts.includes(notification.episodeId)
  );
  const sortedEpisodeInteractions = sortByDate
    ? [...episodeInteractions].sort(
        (a, b) => a.dateListened.getTime() - b.dateListened.getTime()
      )
    : [...episodeInteractions].sort(
        (a, b) => b.dateListened.getTime() - a.dateListened.getTime()
      );
  ...
  return (
    <>
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        {/* Blurred background for the overlay */}
        <ModalContent
          boxShadow="dark-lg"
            ...
            <Box display="flex" justifyContent="center" alignItems="center">
              ...
              >
                <Select
                  value={filter}
                  onChange={(e) =>
                    setFilter(e.target.value as "all" | "episode" | "user")
                  }
                >
